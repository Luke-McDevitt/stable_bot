...
**Last revised:** 2026-04-26 (v5 — Pi 5 16 GB migration locked in;
OAK depth = truth, classical lecture techniques run in parallel for
comparison only; calibrate-via-GUI-button flow; chessboard generator
shipped).
...
Calibration runs **on the Pi** (where the OAK is plugged in), but
the user-facing capture flow is via a button in the laptop's GUI —
the Pi is headless. `scripts/calibrate_oak.py` exposes a small ROS 2
service interface that the GUI calls:

  - `/calibrate/capture_frame` (Trigger) — grab one synced
    chessboard pair (Stage A) or the leveled-platform ArUco frame
    (Stage C). The script auto-detects the chessboard / board and
    accepts or rejects the frame, returning a status to the GUI
    (success: "frame N/30 captured", failure: "chessboard not detected").
  - `/calibrate/solve` (Trigger) — runs `cv2.calibrateCamera` /
    `cv2.stereoCalibrate` / `cv2.stereoRectify` on the captured set
    and writes the YAML if the acceptance gates pass.
  - `/calibrate/reset` (Trigger) — clears captured-frame buffer.

The GUI's Vision Debug panel shows the live OAK feed with detected
chessboard corners drawn in real time, plus a counter "N/30 frames"
and the three buttons above. The chessboard geometry comes from
`chessboard.yaml` produced by `generate_chessboard.py` so changing
the printed board doesn't require code edits.
...
- Telemetry strip:
  ...
  - **Method comparison** (live, mm): OAK-vs-mono, OAK-vs-stereo,
    mono-vs-stereo discrepancies. Color-coded green/yellow/red.

**Calibration controls** (collapsible sub-panel, hidden during demos):

- Live OAK left-eye feed with detected chessboard / ArUco board
  corners drawn in real time (so the user can see whether the
  current pose is being accepted).
- Frame counter "N/30 captured" for Stage A.
- Three buttons:
  - **Capture frame** — calls `/calibrate/capture_frame`. Result
    toast: "frame N/30 captured" or "chessboard not detected, try
    again" or "pose too similar to a captured one — vary angle".
  - **Solve** — calls `/calibrate/solve`. Disabled until ≥ 30 frames.
    Returns reprojection error per eye + epipolar error; refuses
    to write the YAML if acceptance gates fail.
  - **Reset** — clears the buffer.
- Stage selector: "Stage A (intrinsics)" / "Stage B (stereo)" /
  "Stage C (camera-to-world)". Stage C is the one re-run after
  every camera move; the others run rarely.
...
### Calibration services (Stage A/B/C, called by the GUI)

| Service | Type | Notes |
|---|---|---|
| `/calibrate/capture_frame` | std_srvs/Trigger | One frame per click; rejects bad poses. |
| `/calibrate/solve` | std_srvs/Trigger | Runs the OpenCV solver, writes YAML if gates pass. |
| `/calibrate/reset` | std_srvs/Trigger | Clears the captured-frame buffer. |
| `/calibrate/stage` | std_srvs/SetBool style or custom | Select A / B / C. |
...
| Q27 | Vision strategy | OAK onboard depth = ground truth (controller closes on it). Lecture-6 classical methods (homography, F/E + 8-point, triangulation, SfM) run in parallel as comparison estimators with no control authority. |
...
| Q29 | Calibration capture flow | GUI buttons (Capture / Solve / Reset) calling Pi-side ROS services — Pi is headless so terminal cv2.imshow is a non-starter. Live corner overlay in the GUI's existing left-eye feed. |
| Q30 | Chessboard generator | Shipped (`stewart_vision/scripts/generate_chessboard.py`). Default 10x7 squares (9x6 internal corners), 25 mm/square, fits letter landscape. |

### Still open

None — spec locked at v5 (2026-04-26).
